...
def main(model_args: ModelConfig, train_args: TrainConfig, parallel_args: ParallelConfig):
    ...
    for epoch in range(train_args.epochs):
        # a dummy seqlen=200 thing, forward and backward test
        # ids: [B, T] = [64, 200]
        # for _ in range(10):
        #     ids = torch.randint(0, vocab_size, (train_args.batch_size, 140), device=device)
        #     stack_tape = torch.randint(0, model_args.max_stack_depth, (train_args.batch_size, 140, 140), device=device)
        #     attachment_labels = torch.randint(0, model_args.max_stack_depth, (train_args.batch_size, 140), device=device)
