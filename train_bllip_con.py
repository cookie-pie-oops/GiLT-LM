...
def main(model_args: ModelConfig, train_args: TrainConfig, parallel_args: ParallelConfig):
    ...
    model.load_state_dict(torch.load(f"./ckpt/{train_args.run_name}/best/model.pt"))
    model.eval()
    with torch.no_grad():
        test_losses_w = []
        test_losses_a = []
        ...
        # save test result
        wandb.log({
            "test_loss_joint": test_loss,
            "test_loss_word": test_loss_w,
            "test_loss_attachment": test_loss_a,
            "test_joint_ppl": all_ppl.item(), # NOTE: Joint PPL
            "test_word_ppl": word_ppl.item(),
        }, step=general_step + 1) # log test loss and ppl
